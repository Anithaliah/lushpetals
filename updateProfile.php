...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['saveChanges'])) {
    ...
    if (!empty($_POST['oldPassword']) && !empty($_POST['newPassword']) && !empty($_POST['confirmPassword'])) {
        ...
        if ($oldPassword == $storedPassword) {
            ...
            if ($newPassword === $confirmPassword && preg_match($passwordPattern, $newPassword)) {
                echo "New password meets the criteria"; // Debug statement
                ...
                echo "<br> Please make sure New password and confirm password match and <br> Password must be at least 8 characters long and include at least one uppercase letter, one lowercase letter, one number, and one special character (!@#$%^&*()_+{}[]:;<>,.?~\\-).";
                die();
            }
        } else {
...
if (!empty($_FILES['newPicture']['name'])) {
   ...
   // Handle other updates (email, phone) as needed
    ...
    $phoneNumberPattern = '/^\d{1,111}$/';
    if (preg_match($phoneNumberPattern, $newPhone)) {
        echo "Phone number must contain only numeric characters (0-9) and have a maximum length of 111"; // Debug statement
        ...
        echo "Phone number must contain only numeric characters (0-9) and have a maximum length of 11.";
        die();
    }
}
        ...
        <nav class="custom-navbar navbar navbar-expand-md navbar-dark bg-dark" aria-label="Furni navigation bar">
            ...
            <div class="collapse navbar-collapse" id="navbarsFurni">
                ...
                <ul class="custom-navbar-cta navbar-nav mb-2 mb-md-0 ms-5">
                    ...
                    <li><a class="nav-link" href="log.php"><img class="nav-icon" src="images/logout.png" style="width: 24px; height: 24px;"></a></li>
                </ul>
            </div>
        </nav>
    </div>
