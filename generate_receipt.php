...
if ($AccountID !== null) {
    ...
    // Retrieve SubscriptionType from the subscription table
    $subscriptionQuery = "SELECT SubscriptionType FROM subscription WHERE AccountID = $AccountID";
    $subscriptionResult = $conn->query($subscriptionQuery);

    if ($subscriptionResult->num_rows > 0) {
        // Fetch the SubscriptionType
        $subscriptionRow = $subscriptionResult->fetch_assoc();
        $subscriptionType = $subscriptionRow['SubscriptionType'];
    } else {
        // Default SubscriptionType if not found
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<p><strong>Subscription Type:</strong> ' . $row["SubscriptionType"] . '</p>'; // Display the correct Subscription Type
    ...
    // Calculate additional charges based on SubscriptionType
    if ($subscriptionType == "basic") {
        // No additional charges or discounts for basic subscription
        $totalAmount = $subtotal + $deliveryCharge + $serviceCharge;
    } elseif ($subscriptionType == "standard") {
        $totalAmount = $subtotal + $deliveryCharge + $serviceCharge - 10; // Deduct $10 for standard subscription
